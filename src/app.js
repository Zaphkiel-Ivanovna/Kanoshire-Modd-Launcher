...
const UpdateWindow = require('./assets/js/windows/updateWindow');
const MainWindow = require('./assets/js/windows/mainWindow');
...
ipcMain.on('update-window-dev-tools', () => (dev ? UpdateWindow.getWindow().webContents.openDevTools({ mode: 'detach' }) : null));
...
ipcMain.handle('Microsoft-window', async (_, clientId) => {
  const account = await new Microsoft(clientId).getAuth();
  return account;
});
